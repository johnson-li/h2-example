...
from h2.events import (
    ConnectionTerminated, DataReceived, RequestReceived, StreamEnded, SettingsAcknowledged
)
from h2.errors import ErrorCodes
from h2.exceptions import ProtocolError
from basic import RESOURCE
...
class H2Protocol(asyncio.Protocol):
    ...
    def server_push(self):
        ...
        self.conn.push_stream(1, 2, request_headers)

    def request_received(self, headers: List[Tuple[str, str]], stream_id: int):
        print("request received: " + str(stream_id) + " " + str(headers))
    ...
    def stream_complete(self, stream_id: int):
        ...
        """
        print("stream complete " + str(stream_id))
        try:
        ...
        path = headers[':path']
        data = open(RESOURCE + path).read().encode()
        response_headers = (
            (':status', '200'),
...
coro = loop.create_server(H2Protocol, '0.0.0.0', 8443, ssl=ssl_context)
